...
function Update_rest_contr($pdo, $restId, $name, $address, $category, $fileName_one, $fileTmpName_one, $fileSize_one, $fileError_one, $fileType_one, $menu_url) {
    $updateImage = false;
    $imageData_one = null; // Initialize imageData_one to null
    ...
    // If the image was not updated, retain the previous image data
    if (!$updateImage) {
        $result = get_rest_detail($pdo, $restId);
        if ($result && isset($result['image_data_one'])) {
            $imageData_one = $result['image_data_one'];
        }
    }

    // Proceed with the update with checking if the image was uploaded or not
