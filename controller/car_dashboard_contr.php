...
function  delete_car_by_id($pdo, $carId){
    $rowCount = delete_a_car_by_ids($pdo, $carId);
    if ($rowCount > 0) {
        echo "<script>alert('Car deleted successfully.');window.location.href = '../views/car_dashboard/read_car.php';</script>";
    } else {
        echo "<script>alert('Not deleted successfully.');window.location.href = '../views/car_dashboard/read_car.php';</script>";
    }  
}
