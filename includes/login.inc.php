...
if($_SERVER['REQUEST_METHOD'] === "POST"){
    ...
    try{
        ...
        if ($_SESSION["role"] == "rest") {
            header("Location: ../views/restaurant_dashboard/rest_info.php?login=success");
        ...
        if ($_SESSION["role"] == "hotel") {
            header("Location: ../views/hotel_dashboard/company_info.php?login=success");
        ...
        else if  (isset( $_SESSION['redirect_url_hotel'])) {
            
            $redirect_url_guide = filter_var( $_SESSION['redirect_url_hotel'] , FILTER_SANITIZE_URL);
            
            // Clear the redirect URL from the session
            unset($_SESSION['redirect_url']);
            
            // Redirect to the intended URL
            header("Location: $redirect_url_hotel");
            $pdo= null;
            $stmt =null;
            die();
        }    
